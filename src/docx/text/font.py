...
class Font(ElementProxy):
    ...
    def character_spacing(self) -> Length | None:
        ...
        The spacing between characters, in English Metric Units. Positive values expand
    ...
    def kerning(self) -> Length | None:
        ...
        The minimum font size for which kerning is automatically applied, as a |Length|
        value. |None| indicates kerning is not enabled (inherited from style hierarchy).
        """
        rPr = self._element.rPr
        if rPr is None:
            return None
