...
class Table(StoryChild):
    ...
    @property
    def element(self) -> CT_Tbl:
        """Public alias for the underlying ``w:tbl`` element.

        Exposes :attr:`_element` / :attr:`_tbl` without the private-attribute
    ...
    @property
    def next_block(self) -> "Paragraph | Table | None":
        """The next sibling ``w:p`` or ``w:tbl`` block, or |None| at end-of-container.

        Skips non-block siblings like ``w:sectPr`` or bookmark markers.

        .. versionadded:: 2026.05.0
        """
        from docx.text.paragraph import _next_block_sibling

        return _next_block_sibling(self._tbl, self._parent)

    @property
    def previous_block(self) -> "Paragraph | Table | None":
        """The previous sibling ``w:p`` or ``w:tbl`` block, or |None| at start-of-container.

        .. versionadded:: 2026.05.0
        """
        from docx.text.paragraph import _previous_block_sibling

        return _previous_block_sibling(self._tbl, self._parent)
...
class _Cell(BlockItemContainer):
    ...
    @property
    def element(self) -> CT_Tc:
        """Public alias for the underlying ``w:tc`` element.

        Exposes :attr:`_element` / :attr:`_tc` without the private-attribute
        access warnings generated by pyright. Use this when you need to drop
        into the oxml layer (closes upstream#1445).
