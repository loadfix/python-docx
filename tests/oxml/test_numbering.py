...
class DescribeCT_Num:
    ...
    def it_knows_its_numId(self):
        num = cast(CT_Num, element("w:num{w:numId=7}"))
        assert num.numId == 7

    def it_can_add_a_lvlOverride(self):
        num = cast(CT_Num, element("w:num{w:numId=1}/w:abstractNumId{w:val=0}"))
        lvl_override = num.add_lvlOverride(ilvl=2)
        assert lvl_override is not None
        assert lvl_override.ilvl == 2

    def it_can_construct_a_new_num_element(self):
        num = CT_Num.new(num_id=3, abstractNum_id=5)
        assert num.numId == 3
        assert num.abstractNumId.val == 5


class DescribeCT_NumLvl:
    """Unit-test suite for `docx.oxml.numbering.CT_NumLvl`."""

    def it_knows_its_ilvl(self):
        numLvl = cast(CT_NumLvl, element("w:lvlOverride{w:ilvl=3}"))
        assert numLvl.ilvl == 3

    def it_can_add_a_startOverride(self):
        numLvl = cast(CT_NumLvl, element("w:lvlOverride{w:ilvl=0}"))
        startOverride = numLvl.add_startOverride(val=42)
        assert startOverride is not None
        assert startOverride.val == 42
...
class DescribeCT_NumPr:
    ...
    def it_provides_access_to_its_numId_child(self):
        numPr = cast(CT_NumPr, element("w:numPr/w:numId{w:val=5}"))
        assert numPr.numId is not None
        assert numPr.numId.val == 5
...
class DescribeCT_Numbering:
    ...
    def it_can_find_a_num_by_numId(self):
        ...
        num = numbering.num_having_numId(5)
        assert num.numId == 5
