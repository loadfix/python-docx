"""Per-entry zip-package diff for the docx byte-round-trip harness.

Word's authoring floor requires *byte-identical* round-trip on
unmodified reads for the ``.office.docx`` reference fixtures — anything
less means the writer is silently perturbing user bytes. This module
provides the diff primitive the conformance tests run against.

Why per-entry, not whole-zip-hash
---------------------------------

A whole-zip SHA would be fooled by cosmetic differences the OPC writer
is explicitly allowed to perturb: zip-entry ordering,
``zip_date_time``, compression level. The reproducible-zip writer in
...
naming the guilty part (``word/document.xml``, ``word/styles.xml``,
etc.) instead of "hash differs".

What counts as "equal"
----------------------

The byte-round-trip contract is literal: same zip entry name, same
bytes, same count. We deliberately do **not** canonicalise XML before
comparing — attribute-order preservation is part of the contract
(xmlchemy descriptor setters must not re-sort — see
``python-docx/CLAUDE.md``). Canonicalisation would mask the very bug
the harness is meant to catch.
...
Different contract → different primitive. The vsdx harness
(``python-vsdx/tests/conformance/diff.py``) is the model this module
mirrors.
...
class EntryDiff:
    ...
    Attributes:
        name: The zip-entry name (e.g. ``word/document.xml``).
...
@dataclass
class RoundtripDiff:
    """Result of comparing an original and a saved ``.docx`` package.
...
def read_zip_entries(
    ...
    if isinstance(source, (bytes, bytearray)):
        buf: Union[str, io.BytesIO] = io.BytesIO(bytes(source))
...
def render_entry_preview(data: bytes, limit: int = 200) -> str:
    ...
    XML-shaped payloads (anything that decodes as UTF-8) get the first
    ``limit`` characters of their text; binary payloads get a compact
    hex dump of the first 32 bytes with the total length appended. The
    goal is a readable ~one-line hint, not a full diff dump.
    """
    if not data:
        return "<empty>"
    try:
...
def _describe_change(name: str, original: bytes, saved: bytes) -> EntryDiff:
    return EntryDiff(
        name=name,
...
def round_trip_pair(path: Union[str, Path]) -> tuple[bytes, bytes]:
    """Load a ``.docx`` at ``path`` via docx, save it, return both bytes.
    ...
    """
    # Imported lazily so this module doesn't hard-require docx at
